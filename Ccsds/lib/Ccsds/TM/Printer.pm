...
sub TMPrint {
    ...
    if (
        defined(
            my $Time_Packet =
              $Src_Packet->{'Packet Data Field'}->{'Time Packet'}
        )
      )
    {
    ...
    switch ( join( ',', $Pus_Type, $Pus_SubType ) ) {
        case /3,10|3,12/ {
            ...
            print
"HK/Diag Report Definition  Sid=$SID  ColInt=$Col  Status=$Status  NPar=$NPar\n";
        ...
        case '6,6' {
            ...
            print "Memory Id:$MemId  StartAddress:"
              . sprintf( '%08x', $StartAddress )
              . "  Length: $Length Data:\n";
            my $l = 32;
            for ( my $i = 0 ; $i < $Length ; $i += $l ) {
                my $j = $i + $l - 1;
                $j = $Length - 1 if ( $j > $Length - 1 );
                print sprintf( '%08x : ', $i )
                  . join( ' ', map { sprintf '%02X', $_ } @$dta[ $i .. $j ] )
                  . "\n";
            }
        }
        case '8,140' {
            ...
            switch ($FID) {
                case 103 {
                      ...
                      )
                    {
                        ...
                        my @mkeys_s = (
                            'au',     'auRed', 'tmEnc', 'pmAct',
                            'wd',     'rm',    'rmRed', 'buttrRed',
                            'pmBit1', 'sdram', 'pmA',   'pmB',
                            'pmBit0', 'eeprom'
                        );
                        my $sts_print = '';
                        foreach my $mkey_s (@mkeys_s)
                        {  # We print either " +XX" or the same number of spaces
                            $sts_print .=
                              ( $cEntry->{$mkey_s} ) ? ' +PMBact' : ' +PMAact',
                              next
                              if ( $mkey_s eq 'pmAct' );
                            $sts_print .=
                              ( $cEntry->{$mkey_s} )
                        ...
                        my @mkeys_c = (
                            'WDA',     'WDB',   'EPA',    'EPB',
                            'BatA',    'BatB',  'Thr',    'SunLoss',
                            'TempCtl', 'PMAhw', 'PMAall', 'PMAuv',
                            'PMAsw',   'PMBhw', 'PMBall', 'PMBuv',
                            'PMBsw',   'SelPM', 'Sep1',   'Sep2',
                            'Sep3',    'WDen'
                        );
                        my $cond_print = '';
                        foreach my $mkey_c (@mkeys_c) {
                            $cond_print .=
                              ( $cEntry->{$mkey_c} ) ? ' +SelPMB' : ' +SelPMA',
                              next
                              if ( $mkey_c eq 'SelPM' );
                        ...
                        print 'Pattern No        :      '
                          . sprintf( '%04d', $cPat ) . "\n";
                        print 'Status Input      :0x'
                          . sprintf( '%08x', $cSts )
                          . " $sts_print\n";
                        print 'Conditionned Alarm:  0x'
                          . sprintf( '%06x', $cCond )
                          . " $cond_print\n";
                        print 'Attempt Index     :  '
                          . sprintf( '%8d', $cAtmp ) . "\n";
        ...
        case '8,141' {
            ...
            my $Length     = $Pus_Data->{'Length'};
            print
"Sliced Function Report : Function ID=$FID  Slice:$NrCurSlice/$TotSlice Length of Datas:$Length\n";
            my $dta = $Pus_Data->{'Data'};
            if ( $FID == 152 ) {
                print Dumper($dta);
            }
        ...
        case '12,11' {
            ...
            for ( my $i = 0 ; $i < $N ; $i++ ) {
                ...
                print
"ParamId=$ParamId  Mask=$Mask  Param Value=$ParamValue  Limit crossed=$LimCrossed  Previous Chk Status=$PrevCheckSts  Current Chk Status=$CurCheckSts  OBT=$OBT\n";
            }
        }
        case '14,4' {
            my $N1 = $Pus_Data->{'N1'};
            ...
            for ( my $i = 0 ; $i < $N1 ; $i++ ) {
                ...
                my $FStat = $cTM->{'FStat'};
                my $N2    = $cTM->{'N2'};
                print sprintf( '%12s', $PID )
                  . "  FStat=$FStat  Number of Type(N2)=$N2\n";
                for ( my $j = 0 ; $j < $N2 ; $j++ ) {
                    ...
                    my $N3     = $cLType->{'N3'};
                    print
"\tType=$Type  FStat=$TFStat  Number of SubType(N3)=$N3\n";
                    for ( my $k = 0 ; $k < $N3 ; $k++ ) {
        ...
        case '15,6' {
            ...
            for ( my $i = 0 ; $i < $N1 ; $i++ ) {
                ...
                my $N2       = $cTM->{'N2'};
                print sprintf( '%12s', $PID )
                  . "  StoreId1=$StoreId1  Number of Type(N2)=$N2\n";
                for ( my $j = 0 ; $j < $N2 ; $j++ ) {
                    my $cLType   = $cTM->{'Types'}->[$j];
                    my $Type     = $cLType->{'Type'};
                    ...
                    my $N3       = $cLType->{'N3'};
                    print
"\t\tType=$Type  StoreId2=$StoreId2  Number of SubType(N3)=$N3\n";
                    for ( my $k = 0 ; $k < $N3 ; $k++ ) {
                        my $cLSubType = $cLType->{'SubTypes'}->[$k];
                        my $SubType   = $cLSubType->{'SubType'};
                        my $StoreId3  = $cLSubType->{'StoreId3'};
                        print
                          "\t\t\t   SubType=$SubType  StoreId3=$StoreId3 \n";
                    }
                }
            }
        ...
        case '15,13' {
            ...
            for ( my $i = 0 ; $i < $N ; $i++ ) {
                ...
                print
"StoreId $cStoreId  Time:[${cStoreTime1}s .. ${cStoreTime2}s]  Filled: $cPercent%\n";
            }
        }
        case '19,7' {
            my $N = $Pus_Data->{'N'};
