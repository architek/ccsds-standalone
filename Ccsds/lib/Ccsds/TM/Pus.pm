...
our $pus_AckOk =
  Struct( 'AckOk', UBInt16('TC Packet Id'), UBInt16('TC Packet SC') );

our $pus_AckKo = Struct(
    'AckKo',
    ...
    Array(
        sub {
            ( $_->ctx(3)->{'Packet Header'}->{'Packet Sequence Control'}
                  ->{'Source Data Length'} - 6 ) / 4;
        },
        UBInt32('Params')
    )
);
...
our $pus_DirMil =
  Struct( 'Pus_DirMil', UBInt16('Command Word'), UBInt16('Milbus Status'),
    UBInt16('Dsize'), Array( sub { $_->ctx->{'Dsize'} }, UBInt16('MilData') ) );
...
our $pus_hk_report_definition = Struct(
    'Pus HK Report Definition',
    UBInt8('Sid'),
...
our $pus_enabled_events = Struct( 'Enabled events',
    UBInt16('NEid'), Array( sub { $_->ctx->{'NEid'} }, UBInt16('Eids') ) );

#TODO Sub SAU
our $pus_memory_dump =
  Struct( 'Memory Dump', UBInt16('Memory Id'), UBInt32('Start Address'),
    UBInt16('Length'), Array( sub { $_->ctx->{'Length'} }, UBInt8('Data') ) );

our $pus_memory_chk =
  Struct( 'Memory Check', UBInt16('Memory Id'), UBInt32('Start Address'),
    UBInt16('Length'), UBInt16('Crc') );

our $pus_function_status = Struct(
    'Function Status',
    UBInt8('N'),
    Array(
        sub { $_->ctx->{'N'} },
        Struct(
            'Function Info',
            UBInt8('Function Id'),
            BitStruct( 'Status', Padding(7), Flag('Execution Status'), ),
            Enum(
                UBInt8('Function Type'),
                Normal                => 0,
                Vital                 => 1,
...
#Example
our $sw_error_log = Struct(
    'SW Error Log',
    UBInt16('SErr_Count'),
    Array(
        sub { $_->ctx->{'SErr_Count'} },
        Struct(
            'Errors',
            ...
            Value(
                'Time',
                sub {
                    $_->ctx->{'SERR_LOG_SW_ERR_TIMESTAMP_S'} +
                      $_->ctx->{'SERR_LOG_SW_ERR_TIMESTAMP_SUBS'} / 65535;
                }
            ),
...
our $pus_sliced = Struct(
    ...
    Switch(
        'Data',
        sub { $_->ctx->{'Function Id'} },
        { 152 => $sw_error_log, },
        default => Array( sub { $_->ctx->{'Length'} }, UBInt8('Params') )
    )
);

our $pus_function = Struct(
    'Data',
    UBInt8('Function Id'),
    Switch(
        'Data',
...
our $pus_detailed_schedule = Struct(
    'Detailed Schedule',
    UBInt16('N'),
    Array(
        sub { $_->ctx->{'N'} },
        Struct(
            ...
            Array(
                sub {
                    $_->ctx->{'Packet Header'}->{'Packet Sequence Control'}
                      ->{'Source Data Length'};
                },
                UBInt8('TC Application Data')
            ),
...
our $pus_summary_schedule = Struct(
    'Summary Schedule',
    UBInt16('N'),
    Array(
        sub { $_->ctx->{'N'} },
        Struct(
            'TC',
            $Sat_Time,
            BitStruct( 'Pidb', Padding(1), $Pid ),
            BitStruct( 'SSCb', Padding(2), BitField( 'Source Seq Count', 14 ) ),
        )
    )
);

our $pus_command_schedule_status = Struct(
...
my $mon_criteria_lim = Struct( 'Monitoring Criteria on Limits',
    UBInt32('Low Limit'), UBInt16('EID_l'), UBInt32('High Limit'),
    UBInt16('EID_h') );
my $mon_criteria_exp = Struct(
    'Monitoring Criteria on Expected',
    UBInt32('Mask'),
...
our $pus_current_monitoring_list = Struct(
...
our $pus_current_monitoring_oo_list = Struct(
    'Current Monitoring OO List',
    UBInt16('N'),
    Array(
        sub { $_->ctx->{'N'} },
        Struct(
...
our $pus_check_transition = Struct(
    'Check Transition',
    UBInt16('N'),
    Array(
        sub { $_->ctx->{'N'} },
...
our $pus_enabled_tm_sourcepacket = Struct(
    'Enabled TM sourcepacket',
    UBInt8('N1'),
    Array(
        sub { $_->ctx->{'N1'} },
        Struct(
            ...
            Array(
                ...
                Struct(
                    ...
                    Array(
                        sub { $_->ctx->{'N3'} },
                        Struct(
                            'SubTypes', UBInt8('SubType'), UBInt8('FStat')
                        )
                    )
                )
            )
...
our $pus_enabled_hk = Struct(
    ...
    Array(
        ...
        Struct(
            ...
            Array(
                sub { $_->ctx->{'N2'} },
                Struct( 'Sids', UBInt8('SID'), UBInt8('FStat') )
            ),
        )
    )
);

our $pus_enabled_event = Struct(
    'Enabled Events',
    UBInt8('N1'),
    Array(
        sub { $_->ctx->{'N1'} },
        Struct(
            ...
            Array(
                sub { $_->ctx->{'N2'} },
                Struct( 'Eids', UBInt16('EID'), UBInt8('FStat') )
            ),
        )
    ),
);
our $pus_storage_selection_definition = Struct(
    'Storage Selection definition',
    UBInt8('N1'),
    Array(
        sub { $_->ctx->{'N1'} },
        Struct(
...
our $pus_packet_store_catalogue = Struct(
    'Packet Store Catalogue',
    UBInt8('N'),
    Array(
        sub { $_->ctx->{'N'} },
        Struct(
            'Stores',                    UBInt8('StoreId'),
            UBInt32('Storage Time1'),    UBInt32('Storage Time2'),
            UBInt8('Percentage filled'), Padding(9)
        )
    )
);
our $pus_hk_format = Struct(
    'HK Format',
    UBInt8('N'),
    Array(
        sub { $_->ctx->{'N'} },
        Struct(
            ...
            UBInt8('StoreId'),
            BitStruct(
                'BufferPolb', Padding(7), BitField( 'Buffer Policy', 1 )
            ),
...
our $pus_sid_storage_selection_definition = Struct(
    'Sid Storage Selection definition',
    UBInt8('N1'),
    Array(
        sub { $_->ctx->{'N1'} },
        Struct(
            ...
            Array(
                sub { $_->ctx->{'N2'} },
                Struct( 'Sids', UBInt16('SID'), UBInt8('StoreId') )
            ),
        )
    )
);

our $pus_OBCP_list = Struct(
    'OBCP List',
    UBInt8('NProc'),
    Array(
        sub { $_->ctx->{'NProc'} },
        Struct(
            'Procedures',     UBInt8('ProcedureId'),
            UBInt8('Status'), UBInt8('Position')
        )
    )
);

our $pus_OBCP_dump = Struct(
    'OBCP dump',
    UBInt8('Procedure Id'),
    UBInt8('Procedure Steps'),
    Array(
        sub { $_->ctx->{'Procedure Steps'} },
        Struct(
            'Steps',          UBInt8('Procedure Step'),
            UBInt16('Delay'), $TCSourcePacket
        )
    )
);

#Only mapid 1 are allowed, meaning DFH is always true
our $pus_event_detection_list = Struct(
...
our $pus_parameter_report = Struct(
    'Parameter Report',
    UBInt8('NPar'),
    Array(
        sub {
