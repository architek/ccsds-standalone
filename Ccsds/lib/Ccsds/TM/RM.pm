...
our $RMLogEntry = Struct(
    ...
    UBInt8('AtIdx1'),
    BitStruct( 'AtIdxb', Padding(6), BitField( 'AtIdx2', 2 ), ),
    Value(
        'TimeStamp',
        sub {
            $_->ctx->{'TS1'} +
              ...
              $_->ctx->{'TS3'} << 16 +
              $_->ctx->{'TS4'} << 24 +
              ( $_->ctx->{'TSS1'} + $_->ctx->{'TSS2'} << 8 ) / 65535;
        }
    ),
    Value(
        'Conditioned Alarm',
        sub {
            $_->ctx->{'CA1'} + $_->ctx->{'CA2'} << 8 +
              $_->ctx->{'CA3b'}->{'CA3'} << 16;
        }
    ),
    Value(
    ...
    Value(
        'Attempt index',
        sub { $_->ctx->{'AtIdx1'} + $_->ctx->{'AtIdxb'}->{'AtIdx2'} << 8 }
    ),
    ...
    Value(
        'SunLoss', sub { ( $_->ctx->{'Conditioned Alarm'} & 1 << 7 ) >> 7 }
    ),
    Value(
        'TempCtl', sub { ( $_->ctx->{'Conditioned Alarm'} & 1 << 8 ) >> 8 }
    ),
    Value( 'PMAhw', sub { ( $_->ctx->{'Conditioned Alarm'} & 1 << 9 ) >> 9 } ),
    Value(
        'PMAall', sub { ( $_->ctx->{'Conditioned Alarm'} & 1 << 10 ) >> 10 }
    ),
    Value(
        'PMAuv', sub { ( $_->ctx->{'Conditioned Alarm'} & 1 << 11 ) >> 11 }
    ),
    Value(
        'PMAsw', sub { ( $_->ctx->{'Conditioned Alarm'} & 1 << 12 ) >> 12 }
    ),
    Value(
        'PMBhw', sub { ( $_->ctx->{'Conditioned Alarm'} & 1 << 13 ) >> 13 }
    ),
    Value(
        'PMBall', sub { ( $_->ctx->{'Conditioned Alarm'} & 1 << 14 ) >> 14 }
    ),
    Value(
        'PMBuv', sub { ( $_->ctx->{'Conditioned Alarm'} & 1 << 15 ) >> 15 }
    ),
    Value(
        'PMBsw', sub { ( $_->ctx->{'Conditioned Alarm'} & 1 << 16 ) >> 16 }
    ),
    Value(
        'SelPM', sub { ( $_->ctx->{'Conditioned Alarm'} & 1 << 17 ) >> 17 }
    ),
...
our $RMLog = Struct( 'RMLog', UBInt32('Pointer'), Array( 16, $RMLogEntry ) );
